...
impl AaBB {
    pub fn center(&self) -> Pos2 { Pos2::new((self.min.x + self.max.x) / 2.0, (self.min.y + self.max.y) / 2.0) }
}
...
pub fn contact_test_circle_aabb(
    circle: &Circle,
    aabb: &AaBB,
) -> Option<Contact> {
    ...
    query::contact(
        ...
        &Cuboid::new(Vector2::new((aabb.max.x - aabb.min.x) / 2.0, (aabb.max.y - aabb.min.y) / 2.0)),
        CONTACT_PREDICTION,
    )
