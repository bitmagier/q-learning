...
APPLY_CHECKPOINT_PATH = "checkpoints/itest_ballgame_3x3x4_5_512"

RAW_MODEL_SAVE_PATH = 'saved/ql_model_ballgame_3x3x4_5_512'
TRAINED_MODEL_SAVE_PATH = 'saved/ql_model_ballgame_3x3x4_5_512_trained'
...
# At a later run, when we already have a written checkpoint with learned weights available,
# we load it here, so that the saved model already contains the trained weights.
if os.path.isfile(APPLY_CHECKPOINT_PATH + ".index"):
    print("applying found training checkpoint")
    status = model.checkpoint.read(APPLY_CHECKPOINT_PATH)
    status.expect_partial()
    model_save_path = TRAINED_MODEL_SAVE_PATH
    print("producing a trained SavedModel: ", model_save_path)
else:
    model_save_path = RAW_MODEL_SAVE_PATH
    print("producing a fresh SavedModel with random weights: ", model_save_path)
